...
export const getColorNameFromVarni = (value: string) => {
	const colorsGroup = [
		{
			color_codes: ['#aaaeee', '#aaaeee', '#F33A6A'],
			name: 'Rose'
		},
		{
			color_codes: ['#ffffff', '#E5E4E2'],
			name: 'White'
		},
		{
			color_codes: ['#cccaaa', '#ffff00', '#FFDF00'],
			name: 'Yellow'
		}
	]
...
export const checkVideoURLType = (url) => {
	if (!url) return '' // Return empty string for empty or undefined URLs
	...
	// Check for online video streaming
	const onlineVideoStreamingExtensions = ['.m3u8']
	for (const extension1 of onlineVideoStreamingExtensions) {
		if (url.endsWith(extension1)) {
			return 'online_video_streaming'
		}
	}
	...
	for (const extension2 of videoFileExtensions) {
		if (url.endsWith(extension2)) {
			return 'video_file_extension'
		}
	}

	return '' // If none of the checks match, return empty string
}
