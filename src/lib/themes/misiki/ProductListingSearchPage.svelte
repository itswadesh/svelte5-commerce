...
import { PrimaryButton, ProductSkeleton } from '$lib/ui'
...
const loadCartWishlistDataToProductCard = () => {
	if (browser) {
		...
		// 	origin: $page.data.origin
		// })
		// wishlists = await getWishlistFromStore({
		// 	storeId: $page.data?.store?.id,
		// 	origin: $page.data.origin
		// })
		cartStore.subscribe((value) => {
			if (value?.items?.length && Object.values(value)?.length) {
				cart = value
				const newArray = data.products.data.map((item2) => {
			...
			} else productsArray = data.products.data
		})
...
afterNavigate(() => {
	loadCartWishlistDataToProductCard()
})
...
<div class="h-full min-h-screen">
	...
	{#if !hidden && innerWidth <= 1024 && data.products?.data?.length}
		<button
			...
			<span class="flex-1">{data.products?.data?.length} / {data.products?.count}</span>
		</button>
	{/if}

	<div class="px-3 py-5 sm:p-10 mb-10 sm:mb-20 flex flex-col gap-5">
		<Breadcrumb
			isHomeIcon="{false}"
			categoryPool="{data.product?.category?.pathA}"
			currentProductName="Search" />
		...
		<div class="flex flex-col gap-5">
			<div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
				...
				<div class="flex flex-wrap items-baseline gap-2">
					<h1 class="capitalize font-normal">{data.searchData || 'New Arrivals'}</h1>
				</div>
				...
				{#if data.products?.data?.length}
					<select
						...
						class="max-w-max border bg-transparent p-1 hover:border-zinc-400 focus:outline-none focus:border-zinc-500 focus:ring focus:ring-zinc-300 cursor-pointer"
		...
		<div class="flex flex-col lg:flex-row gap-5 sm:gap-10 items-start">
			{#if data.products.facets}
				<DesktopFilter
					class="hidden lg:block {hellobar?.active?.val ? 'top-32' : 'top-24'}"
			...
			<div class="w-full flex-1">
				...
				{#if data.loading}
					<div>
						<ul
							class="grid grid-cols-2 items-start sm:flex sm:flex-wrap sm:justify-between gap-3 sm:gap-6">
							{#each { length: 10 } as _}
								<li>
									<ProductSkeleton />
								</li>
				...
				{:else if productsArray?.length}
					<ul
						class="grid grid-cols-2 items-start sm:flex sm:flex-wrap sm:justify-between gap-3 sm:gap-6">
						{#each productsArray as p, px (p._id)}
